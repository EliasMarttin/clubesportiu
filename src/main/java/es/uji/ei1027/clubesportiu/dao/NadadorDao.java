...
public class NadadorDao {
    ...
    public void updateNadador(Nadador nadador) {
        jdbcTemplate.update("UPDATE Nadador SET edat =?, num_federat =?, pais =?,genere =? WHERE nom =?",
                nadador.getEdat(),nadador.getNumFederat(),nadador.getPais(),nadador.getGenere(),nadador.getNom());
    }
